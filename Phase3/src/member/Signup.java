...
public class Signup {
    ...
    public void signUp(Statement st) throws SQLException {
        while(true)
        {
            ...
            System.out.println("*** [필수] 회원가입할 아이디를 입력해주세요. ***");
            ...
            System.out.println("*** [필수] 휴대폰 번호를 입력해주세요.(010-XXXX-XXXX) ***");
            String phone = sc.next();
            phone = phone.replaceAll("-", " ");
//            System.out.println(phone);
            acc.setPhone_num(phone);

            System.out.println("*** [선택] 생년월일을 입력해주세요.(YYYY-MM-DD) ***");
            acc.setBirth_date(sc.nextLine());

            sc.nextLine();
            System.out.println("*** [선택] 나이를 입력해주세요. ***");
            String age_temp = sc.nextLine();
            if(!age_temp.equals(""))
                acc.setAge(Integer.parseInt(age_temp));

            System.out.println("*** [선택] 성별을 입력해주세요.(M / F) ***");
            acc.setGender(sc.nextLine().equals("M") ? "man" : "woman");

            System.out.println("*** [선택] 주소를 입력해주세요. ***");
            acc.setAddress(sc.nextLine());

            System.out.println("*** [선택] 직업을 입력해주세요. ***");
            acc.setJob(sc.nextLine());

            if(!acc.isManager()) {
                System.out.println("*** [선택] 멤버등급 입력해주세요. (basic / premium / prime) ***");
                acc.setMem_type(sc.nextLine());
            } else{
        ...
        System.out.println(acc.getAcc_id() + " 님, 반갑습니다 !");
        dbUpdate(st);
    }
}
/*
