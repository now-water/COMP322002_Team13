...
실행 및 사용 방법.
 - 첨부한 Phase4 프로젝트 파일을 열고 터미널 환경에서 'node main'을 입력 후 Server와 Database가 연동이 되면 브라우저를 열어
...
기능 설명
	...
	-> 사용자 로그인 (로그인 이후에는 아이디와 비밀번호가 브라우저 세션 정보에 남아있게 되어 추가적인 로그인을 할 필요가 없게 한다.)
		사용자 로그인 시 관리자 기능(영화 등록 및 수정)이 제한된 화면이 나타나게 되며 우측 상단, 아이디 정보에 관한 메뉴를 클릭하면
		내 정보, 내가 평가한 영화 항목, 로그아웃 기능들 이 있고, 좌측 Movies와 Search탭에는 영화 list와 검색 기능을 제공한다.
			...
			- 평가 항목 : 내가 평가한 영화 항목들을 DB에 질의하여 테이블을 통해 보여준다. 테이블 페이지에서는 페이지 당 record 갯수 설정과 간단한 검색 기능 또한 제공한다.

			- 로그아웃 : 아이디, 비밀번호 등의 세션정보를 모두 반환한 뒤 로그인 화면으로 돌아간다.
	...
	->관리자 로그인 : 당연하게도 관리자 로그인은 사용자 로그인의 모든 기능을 사용할 수 있으며 관리자 로그인 시 왼쪽 탭에 영화 등록과 수정을 위한 register와 modify탭이 추가된다.
